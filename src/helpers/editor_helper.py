...
def render_ad_on_frame(app_state: APPState, frame_idx: int) -> np.ndarray:
    ...
    Assumptions:
    - app_state.video.frames[frame_idx] is np.ndarray (BGR)
    - app_state.ad.image is np.ndarray (BGR)
    - app_state.ad.corners is (4,2) float32
    - app_state.placement.quad is (4,2) float32
    """
    ...
    # Build mask (where ad exists)
    mask = np.any(warped_ad > 0, axis=2)

    # Composite
    frame[mask] = warped_ad[mask]
