...
#             video=frames,  # SAM3 accepts PIL Images directly
...
#         logger.info(f"Adding box prompt: {initial_box[0]} for object {obj_id}")
        
#         # Add box prompt on first frame using the correct API
#         self.processor.add_inputs_to_inference_session(
#             inference_session=inference_session,
#             frame_idx=0,
#             obj_ids=[obj_id],  # Must be a list
#             input_boxes=[initial_box],  # Format: [[[x1, y1, x2, y2]]]
#         )
        
#         # IMPORTANT: Run inference on first frame to initialize tracking
#         with torch.no_grad():
#             _ = self.model(
#                 inference_session=inference_session,
#                 frame_idx=0,
...
#         logger.info("Initial frame processed, starting propagation...")
...
#                 inference_session=inference_session,
#                 start_frame_idx=0  # Explicitly set start frame
#             ):
...
#                 # ensure 2D
#                 if mask_2d.ndim != 2:
#                     raise ValueError(f"Invalid mask shape after squeeze: {mask_2d.shape}")

#                 mask_img = (mask_2d > 0.5).astype(np.uint8) * 255
